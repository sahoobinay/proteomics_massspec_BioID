...
FeaturePlot(seurat_obj, features = c("Cbfa2t3",  "Glis2", "mCherry", "Luciferase", "Cbfa2t3_Glis2_fusion"), split.by = "sample")

# ADT expression
DefaultAssay(seurat_obj) <- "ADT"

FeaturePlot(seurat_obj, features = c("Ms.CD4", "Ms.CD14"), split.by = "sample")
#----------------------------
...
# Save
write.csv(rna_markers, "WT_vs_KI_RNA_markers.csv")
write.csv(adt_markers, "WT_vs_KI_ADT_markers.csv")

VlnPlot(seurat_obj, features = c("Ms.CD8"), assay = "ADT", group.by = "sample")
